...
# @updates  values taken by Stella that are constant and not err bars were commented out (i.e. surface temp units)
#           spectrum data is now read in as an array of floats insted of individual strings
#           milliseconds since first data point was added. this will be used to compare to gps_point
#           added get_batch, which separates out the desired batch of stella points. takes list of all points and string batch to compare to.
# TODO:     none
...
class StellaPoint:
    def __init__(self, batch, day, timestamp, decimal_hour, milliseconds, surface_temp, surface_temp_error_bar,
                 air_temp_units, air_temp, air_temp_error_bar, relative_humidity, relative_humidity_error_bar, air_pressure_hpa, air_pressure_error_bar, altitude_m_uncal, altitude_error_bar, visible_spectrum_error_bar, vs_pows, nir_spectrum_error_bar,
                 nir_pows):
        self.batch = batch
        self.day = day
    ...
    def print_stella(self):
            print(self.batch, self.day, self.timestamp, self.dh, self.ms, self.surface_temp, self.st_error_bar, self.air_temp, self.at_error_bar, self.rel_humid, self.rh_error_bar, self.air_pressure_hpa, self.ap_error_bar, self.altitude_m_uncal, self.altitude_error_bar, self.vs_error_bar, self.vs_pows, self.nir_spectrum_error_bar, self.nir_pows)
